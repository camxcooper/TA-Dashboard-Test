...
const Charts = (() => {
  ...
  function fmt(n) {
    ...
    if (n >= 1000) return (n >= 10000 ? "$" : "") + (n / 1000).toFixed(0) + "K";
  ...
  function sparkline(container, data, color = COLORS.purple, filled = true) {
    ...
    if (filled) {
      ...
      grad.innerHTML = `<linearGradient id="sg_${color.replace("#","")}" x1="0" y1="0" x2="0" y2="1">
  ...
  function funnel(container, stages) {
    ...
    stages.forEach((stage, i) => {
      ...
      svg.appendChild(svgEl("rect", {
        ...
        rx: 6, fill: color,
        "fill-opacity": Math.max(0.55, alpha),
      }));

      const label = svgEl("text", {
        x: x - 10, y: y + barH / 2 + 5,
        "text-anchor": "end", "font-size": "12",
        fill: COLORS.textLight, "font-family": "inherit",
      });
      ...
      const count = svgEl("text", {
        ...
        fill: "#F1F5F9", "font-family": "inherit",
      });
      ...
      if (stage.conversion !== null) {
        const conv = svgEl("text", {
          x: padL + maxW + 14, y: y + barH / 2 + 5,
          "text-anchor": "start", "font-size": "12",
          fill: color, "font-family": "inherit",
        });
        conv.textContent = `${stage.conversion}% conv`;
        svg.appendChild(conv);
      }

      if (stage.avgDays > 0) {
        const days = svgEl("text", {
          x: padL + maxW + 108, y: y + barH / 2 + 5,
          "text-anchor": "start", "font-size": "11",
          fill: COLORS.text, "font-family": "inherit",
        });
        days.textContent = `${stage.avgDays}d avg`;
        svg.appendChild(days);
      }

      if (i < stages.length - 1) {
        const nextRatio = stages[i+1].count / total;
        const nextW = Math.max(minW, maxW * Math.pow(nextRatio, 0.5));
        const nx = padL + (maxW - nextW) / 2;
        const ny = (i + 1) * (barH + gap);
        svg.appendChild(svgEl("polygon", {
          points: `${x},${y+barH} ${x+barW},${y+barH} ${nx+nextW},${ny} ${nx},${ny}`,
  ...
  function lineChart(container, { labels, series, height = 200, showGrid = true }) {
    ...
    const mn = Math.floor(Math.min(...allVals) * 0.95);
    const mx = Math.ceil(Math.max(...allVals) * 1.05);
    const range = mx - mn || 1;
    ...
    if (showGrid) {
      ...
      for (let t = 0; t <= ticks; t++) {
        const v = mn + (range / ticks) * t;
        const y = py(v);
        svg.appendChild(svgEl("line", { x1: padL, y1: y, x2: W - padR, y2: y, stroke: COLORS.grid, "stroke-width": "1" }));
        const lbl = svgEl("text", { x: padL - 6, y: y + 4, "text-anchor": "end", "font-size": "10", fill: COLORS.text, "font-family": "inherit" });
        lbl.textContent = v >= 10000 ? (v/1000).toFixed(0)+"K" : v >= 1000 ? (v/1000).toFixed(1)+"K" : Math.round(v);
        svg.appendChild(lbl);
      }
    }

    labels.forEach((l, i) => {
      const lbl = svgEl("text", { x: px(i), y: H - 6, "text-anchor": "middle", "font-size": "10", fill: COLORS.text, "font-family": "inherit" });
      lbl.textContent = l;
      svg.appendChild(lbl);
    });
    ...
    series.forEach((s) => {
      ...
      valid.forEach(p => {
        svg.appendChild(svgEl("circle", { cx: p.x, cy: p.y, r: "3", fill: s.color, stroke: COLORS.surface, "stroke-width": "1.5" }));
      });
  ...
  function barChart(container, { labels, series, height = 180, stacked = false, showValues = true }) {
    ...
    const allVals = stacked
      ? labels.map((_, i) => series.reduce((s, sr) => s + (sr.data[i] || 0), 0))
    ...
    [0, 0.25, 0.5, 0.75, 1].forEach(t => {
      ...
      svg.appendChild(svgEl("line", { x1: padL, y1: y, x2: W - padR, y2: y, stroke: COLORS.grid, "stroke-width": "1" }));
      const lbl = svgEl("text", { x: padL - 6, y: y + 4, "text-anchor": "end", "font-size": "10", fill: COLORS.text, "font-family": "inherit" });
      lbl.textContent = v >= 1000 ? (v/1000).toFixed(0)+"K" : v;
      svg.appendChild(lbl);
    });

    labels.forEach((l, i) => {
      const cx = padL + i * groupW + groupW / 2;
      const xLbl = svgEl("text", { x: cx, y: H - 6, "text-anchor": "middle", "font-size": "10", fill: COLORS.text, "font-family": "inherit" });
      xLbl.textContent = l;
      ...
      if (stacked) {
        ...
        series.forEach((s) => {
          ...
          svg.appendChild(svgEl("rect", {
            x: cx - barW / 2, y: yOff - bH,
            width: barW, height: bH, rx: i === 0 ? 4 : 0,
        ...
        series.forEach((s, si) => {
          ...
          svg.appendChild(svgEl("rect", {
            x: bx, y: py(v), width: barW, height: bH, rx: 3,
            fill: s.color, "fill-opacity": "0.85",
          }));
          if (showValues && v > 0) {
            const vLbl = svgEl("text", { x: bx + barW / 2, y: py(v) - 4, "text-anchor": "middle", "font-size": "9", fill: s.color, "font-family": "inherit" });
            vLbl.textContent = v;
  ...
  function donut(container, { segments, size = 140, thickness = 28, label, sublabel }) {
    ...
    segments.forEach((seg, i) => {
      ...
      svg.appendChild(svgEl("path", {
        d, fill: "none", stroke: seg.color || PALETTE[i % PALETTE.length],
    ...
    if (label) {
      const t = svgEl("text", { x: cx, y: cy + 6, "text-anchor": "middle", "font-size": "22", "font-weight": "700", fill: "#F1F5F9", "font-family": "inherit" });
      t.textContent = label;
      svg.appendChild(t);
    }
    if (sublabel) {
      const t = svgEl("text", { x: cx, y: cy + 22, "text-anchor": "middle", "font-size": "11", fill: COLORS.textLight, "font-family": "inherit" });
  ...
  function hBar(container, { items, height = 200, showPct = false, maxVal = null }) {
    ...
    const labelW = 100, valW = 52;
    const barAreaW = W - labelW - valW - 20;
    const mx = maxVal || Math.max(...items.map(it => it.value));
    const svg = makeSVG(W, items.length * (barH + gap));

    items.forEach((item, i) => {
      const y = i * (barH + gap);
      const barW = (item.value / mx) * barAreaW;
      const color = item.color || PALETTE[i % PALETTE.length];

      svg.appendChild(svgEl("rect", {
        x: labelW, y: y, width: barAreaW, height: barH, rx: 4,
        fill: COLORS.grid,
      }));

      svg.appendChild(svgEl("rect", {
        x: labelW, y: y, width: barW, height: barH, rx: 4,
        fill: color, "fill-opacity": "0.85",
      }));

      if (item.secondary !== undefined) {
        const sw = (item.secondary / mx) * barAreaW;
        svg.appendChild(svgEl("rect", {
          x: labelW, y: y + barH * 0.35, width: sw, height: barH * 0.3, rx: 2,
          fill: "#F1F5F9", "fill-opacity": "0.3",
        }));
      }
      ...
      val.textContent = showPct ? item.value + "%" : item.value.toLocaleString();
      svg.appendChild(val);
    });
  ...
  function budgetBars(container, { items, height }) {
    const W = 520, H = height || items.length * 60;
    const labelW = 160, barAreaW = W - labelW - 80;
    const barH = 12, gap = 12;
    const mx = Math.max(...items.flatMap(it => [it.budget, it.actual]));
    const svg = makeSVG(W, H);

    items.forEach((item, i) => {
      const y = i * (barH * 2 + gap + 20) + 10;
      const bW = (item.budget / mx) * barAreaW;
      const aW = (item.actual / mx) * barAreaW;
      ...
      svg.appendChild(svgEl("rect", { x: labelW, y: y, width: barAreaW, height: barH, rx: 3, fill: COLORS.grid }));
      svg.appendChild(svgEl("rect", { x: labelW, y: y, width: bW, height: barH, rx: 3, fill: "#475569", "fill-opacity": "0.6" }));

      svg.appendChild(svgEl("rect", { x: labelW, y: y + barH + 3, width: aW, height: barH, rx: 3, fill: color, "fill-opacity": "0.85" }));

      const pLbl = svgEl("text", { x: labelW + barAreaW + 10, y: y + barH + 3, "font-size": "11", "font-weight": "600", fill: color, "font-family": "inherit" });
      pLbl.textContent = pct + "%";
      svg.appendChild(pLbl);

      const bLbl = svgEl("text", { x: labelW + barAreaW + 36, y: y + barH - 1, "font-size": "10", fill: COLORS.text, "font-family": "inherit" });
      bLbl.textContent = "$" + (item.budget/1000).toFixed(0) + "K";
